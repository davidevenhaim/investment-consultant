...
def build_graph_for_session(
    ...
    Providers/store may be injected for testing; defaults to live implementations.

    Graph order (CLAUDE.md M6):
    LoadTickerContext → RetrieveMemory → FetchMarketData → ComputeSignals
    → FetchFundamentals → NeutralRecommendation → PersonalizedRecommendation
    → PersistResults (Postgres + ChromaDB)
    """
    ...
    builder.add_edge("fetch_fundamentals", "neutral_recommendation")
